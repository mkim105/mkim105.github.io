...
* Project는 Class Library (.NET Framework) 탬플릿으로 만들어야한다. (.NET Core나 .NET Standard)로 만들면 기능이 제한됨(e.g., generating windows form)
...
## Section 2-19. Windows Form을 생성하고 그 안의 User Input을 Revit API로써 사용하는 방법  

Windows Form 생성방법: Visual Studio 2019 기준, File -> Add -> New Project에서 새로운 Windows Form을 생성한다.
Form의 목적: 유저가 Revit model 상에서 오브젝트를 수평 또는 수직 방향으로 정해진 값에 따라 이동시키게 만드는 것 이다.
