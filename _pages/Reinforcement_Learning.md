...
조지아텍의 Reinforcement Learning 수업을 따라가면서 공부하는 자료를 정리하는 시리즈를 시작합니다. 해당 수업은 Udacity.com에 가시면 무료로 수강하실 수 있습니다. (https://www.udacity.com/course/reinforcement-learning--ud600) 동영상 강의 수강과 함께 Sutton & Barto 교수님의 Reinforcement Learning 2nd Edition 책을 요약하는 위주로 진행되며, [강화학습-1], [강화학습-2] 식으로 시리즈 포스팅을 할 것인데, 반드시 챕터 순서대로 진행되지는 않고, 조지아텍 자체 석사 강의 Syllabus에서 가이드하는 순서를 따릅니다. 예를 들어 첫 주에는 Sutton 교수님 책의 챕터 1, 3, 16을 읽습니다.

첫 포스팅에서는 Sutton & Barto 교수님의 Reinforcement Learning 2nd Edition 책의 제 1과: Introduction를 살펴보도록 하겠습니다. 주로 요약 위주로 적지만 제가 주석을 단 부분들이 있습니다.
...
agent: 에이전트
  * 행동하고 선택하는 주체를 말합니다. 체스의 경우 체스 플레이어, 로봇 문제의 경우 로봇이 에이전트입니다.

credit assignment:
  * 나중에 보상이 주어졌을 때, 과연 그 보상이 있기까지 행한 숱한 행동 중에 어느 것이 정말 보상을 이끌었는지 판별해내야하는 문제(e.g., 체스의 경우 게임이 끝나봐야지 이겼는지 졌는지 알 수 있음)
  
environment: 환경
  * 에이전트와 상호 작용하는, 에이전트 외부의 모든 요소들이 환경입니다.

exploration and exploitation: 탐색과 이용
  * exploration: 최적이 아닌 선택을 해서 더 많은 데이터를 탐색(exploration)하여 더 나은 모델을 학습해볼 가능성을 여는 것.
  * exploitation: 에이전트가 경험하고 학습한 모델이 제시하는 최적의 선택(exploitation)을 하는 것.
  
model: (환경의)모델 
  * 실제 환경을 단순화하고 각 환경의 요소들이 어떻게 작용(dynamics)하는지를 정의한 것입니다. 
  * 이 책에서는 많은 문제들을 MDP(Markov Decision Process)라는 형태로 모델링합니다. 
  * model-based 방식: 이러한 모델들을 이용해서 최적 행동 정책을 계산하는 알고리즘
  <->
...
reward signal: 보상 시그널
  * 해당 강화학습 문제의 목적을 정의합니다. 
  * 높은 보상이 있는 쪽으로 에이전트는 행동 정책을 수정해 나가야 합니다. 이 보상 시그널 또한 확률적인 시그널일 수 있습니다. (예를 들어 이 문을 열면 40% 확률로 6의 보상이 있고, 60% 확률로 -3의 보상이 있는 식)
  
state: 상태
  * 환경의 여러 가지 형태입니다. 체스 문제의 경우 체스판과 그 위 장기말들이 환경을 이루고, 장기말이 어떤 식으로 배열되어 있는지가 상태를 정의합니다. 
    * 체스판의 상태가 이런 배열이면 이런 행동을 하고, 저런 배열이면 저런 행동을 하도록 우리는 학습을 진행하는데, 여기서 이런 배열, 저런 배열 등이 모두 각기다른 상태입니다.

value function: 가치 함수
  * 해당 상태(state)가 장기적으로 갖는 효용입니다. 
  * 보상 시그널이 단기적인 효용이라면, 가치 함수는 해당 상태(state)에서 미래에 가질 수 있는 보상 시그널들까지 고려한, 조금 더 장기적인 효용을 본다고 생각하시면 됩니다. 챕터 3에서 자세히 설명합니다.
...
출처:[irealist.org](https://irealist.org/data-science/?uid=7864&mod=document&pageid=1)
